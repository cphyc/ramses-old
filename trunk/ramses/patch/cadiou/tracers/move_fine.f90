...
subroutine move1(ind_grid, ind_part, ind_grid_part, ng, np, ilevel, flux)
 ...
 ! Compute parent cell position
  ...
  if (MC_tracer) then
     do j = 1, np
        !! This is a tracer, move it according to flux
        ! Get the cell in which we are
        ii=0; ij=0; ik=0 ! TODO define them depending on whether we're on the right side of the grid or left side

        outflux = 0d0
        do idim = 1, ndim
           ! TODO: timestep issues!
           if (flux(j, ii, ij, ik, 1, idim) > 0) outflux = outflux + flux(j, ii, ij, ik, 1, idim)
        end do

        ! Break if no outflux
        if (outflux == 0d0) continue

        escape_proba = outflux / mass_of_cell

        ! Decide randomly if we move the particle
        rand = 0.5D0
        if (rand < escape_proba) then
           ! Ok, I'm moving it. Which direction ?
           rand = 0.5D0
           do direction = 1, 2 ! left right
              do idim = 1, ndim ! TODO: solve trickiness about the ii ij and ik.
                 ! for example for direction=1 and leftmost cell, ii = 0
                 !                           2                    ii = 1
                 ! for example for direction=1 and rightmost cell, ii = 1 ?
                 !                           2                     ii = 2 ?
                 proba_dim = max(0, flux(j, ii, ij, ik, 1, idim)) / mass_of_cell
                 if (rand < proba_dim) then
                    ! now find a suitable location in the neighbour grid (or cell -> TODO)
                 else
  ...
  do idim = 1, ndim
     if(static)then
        ...
        do j = 1, np
           ! Don't overright the position if using MC_tracers
           if (MC_tracer) then
              if(mp(ind_part(j)) == 0d0) continue
           end if
