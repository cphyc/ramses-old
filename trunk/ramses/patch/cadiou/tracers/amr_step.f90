recursive subroutine amr_step(ilevel, icount)
  ...
  real(dp), save :: flux(1:nvector, if1:if2, jf1:jf2, kf1:kf2, 1:nvar, 1:ndim)
  ...
  if (levelmin < nlevelmax .and. (.not. static .or. (nstep_coarse_old == nstep_coarse .and. restart_remap)))then
     ...
     if (ilevel == levelmin) then
        if (nremap > 0) then
           ...
           if (nrestart > 0 .and. first_step) then
              if (nrestart == nrestart_quad) restart_remap=.true.
  ...
  if (hydro)then
     ...
     if (.not.static_gas) call godunov_fine(ilevel, flux)
  ...
  if (pic)then
     ...
     if (static_dm .or. static_stars) then
        ...
        call move_fine(ilevel, flux) ! Only remaining particles
     end if
  end if
  ...
  if (.not.static .or. (nstep_coarse_old == nstep_coarse .and. restart_remap)) call flag_fine(ilevel, icount)
